...
function seedExercises(database) {
  ...
  const insertExercise = database.prepare(`
    INSERT INTO exercises (id, name, measurement, type, note, sets, reps, weight, duration, video)
    VALUES (@id, @name, @measurement, @type, @note, @sets, @reps, @weight, @duration, @video)
...
function hydratePlan(database, plan) {
  ...
  const workouts = database
    ...
    .map((workout) => ({
      ...
      exercises: database
        .prepare(
          `SELECT pwe.*, e.measurement, e.type, e.note, e.video
