...
function seedExercises(database) {
  const exerciseCount = database.prepare("SELECT COUNT(*) AS count FROM exercises").get().count;
  if (exerciseCount > 0) return;

  const raw = fs.readFileSync(path.join(process.cwd(), "defaults202603.json"), "utf8");
  ...
  const insertExercise = database.prepare(`
    ...
    VALUES (@id, @name, @measurement, @type, @note, @sets, @reps, @weight, @duration, @video, @equipment)
  ...
  `);
  const getExercise = database.prepare("SELECT id FROM exercises WHERE name = ?");
  const deleteAliases = database.prepare("DELETE FROM exercise_aliases WHERE exercise_id = ?");
  const insertAlias = database.prepare(`
    INSERT OR IGNORE INTO exercise_aliases (id, exercise_id, alias)
    VALUES (?, ?, ?)
  `);
  ...
  const transaction = database.transaction(() => {
    for (const exercise of exercises) {
      insertExercise.run({
        ...
        equipment: exercise.equipment != null ? parseExerciseEquipment(exercise.equipment) : null,
      });
      const row = getExercise.get(exercise.name);
      deleteAliases.run(row.id);
      for (const alias of exercise.alias || []) {
        insertAlias.run(randomUUID(), row.id, alias);
      }
    }
  });
...
export function updateUser({ id, username, password, role }) {
  ...
  const database = db();
  const existing = database.prepare("SELECT id, role FROM users WHERE id = ?").get(id);
...
function savePlanWorkouts(database, planId, workouts) {
  ...
  const insertExercise = database.prepare(
    `INSERT INTO plan_workout_exercises
     (id, workout_id, exercise_id, exercise_name, position, sets, reps, weight, duration)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  );
  ...
  workouts.forEach((workout, workoutIndex) => {
    ...
    (workout.exercises || []).forEach((exercise, exerciseIndex) => {
      ...
      insertExercise.run(
        ...
        numberOrNull(exercise.reps),
        numberOrNull(exercise.weight),
        numberOrNull(exercise.duration),
      );
    });
  });
}
